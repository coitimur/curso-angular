...
exports.insert=function(oItem,response){
    ...
    pg.connect(db.connect_options, function(err, client,done) {
        ...
        client.query('INSERT INTO webshop.phones_images ('+ sFields+') VALUES('+sFieldsSubt+') RETURNING id', aValues, function(err, result) {
           // handle an error from the query
...
exports.update=function(oItem,response){
   ...
   pg.connect(db.connect_options, function(err, client,done) {
        ...
        client.query('UPDATE webshop.phones_images SET '+sFields+'  WHERE id=$1', aValues, function(err, result) {
...
exports.remove=function(params,response){
    ...
    pg.connect(db_config.connect_string, function(err, client,done) {
        ...
        client.query('DELETE FROM public.phones_images WHERE id=$1', [params.id], function(err, result) {
           // handle an error from the query
              if(handleError(err)) {
                    response(err,null);
